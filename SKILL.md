...
description: |
  通用商旅出行规划技能 v1.1 · 面向所有商务出差人士。
  一站式AI出行规划 + 全平台酒店比价 + 多格式报告输出。
  集成12306 MCP（火车票实时查询）+ 高德地图（路径规划/POI/天气）+ Web搜索（航班/酒店价格）。
  支持**HTML报告 + PDF + 飞书云文档**多格式同步输出，所有预订链接真实可一键跳转。
  触发条件：
...
- 📑 **PDF导出**：支持PDF格式输出，适合打印和差旅报销
- 📝 **飞书云文档**：检测到飞书MCP时自动生成飞书文档（团队协作）
- 🔄 **跨平台兼容**：自动检测AI伙伴能力，降级输出（全格式→HTML→Markdown）
...
**⚠️ 数据真实性规则（核心红线）**：
1. **酒店价格/评分 → 必须实时查询**，通过Web搜索或携程/飞猪链接获取，严禁编造
2. **交通班次/票价 → 通过12306 MCP或Web搜索实时获取**
3. **天气 → 通过高德天气API实时获取**
4. **实时数据获取失败 → 明确告知用户"请点击链接查看实时价格"**，绝不编造占位数据
5. **预算计算 → 基于实时查询结果计算**，不得使用硬编码参考值作为实际价格
6. **HTML报告 → 所有数据标注来源**（12306/携程/高德/Web搜索）
...
搜索策略:
  ...
  2. Web搜索 → 各平台价格对比（必须使用实时数据）
  3. 生成多平台预订链接

⚠️ 价格真实性规则:
  - 必须通过Web搜索获取当前日期的实时价格
  - 如搜索未返回具体价格，构造携程/飞猪搜索链接让用户查看
  - 严禁使用任何硬编码价格或编造"约¥xxx"占位
...
格式选择逻辑:
  1. 检测 feishu-integration MCP → YES: 额外生成飞书云文档
  2. 检测 write_to_file + preview_url → YES: 生成 HTML
  3. 用户要求PDF → 生成 PDF（HTML转PDF）
  4. 以上均无 → 降级为 Markdown 文本
...
| 格式 | 适用场景 | 生成方式 |
|------|----------|----------|
| 📄 **HTML** | 在线预览、预订跳转（**默认**） | write_to_file + preview_url |
| 📑 **PDF** | 打印归档、差旅报销 | HTML → PDF |
| 📝 **飞书云文档** | 团队协作 | feishu-integration MCP |
| 📋 **Markdown** | 降级方案 | 直接文本输出 |
